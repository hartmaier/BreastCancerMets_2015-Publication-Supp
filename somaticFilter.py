...
if args.vcfFile:
    ...
    for record in vcf_reader:
        ...
        for call in record.samples:
            if (re.search(r"[Nn]ormal", call.sample) != None) and (call.called) and (call.gt_type == 0) and (call['DV'] == 0) and (call['DR'] >=20):
                rcRef.append(call['RC'])
            if (re.search(r"[Tt]umor", call.sample) != None) and (call.called) and (call.gt_type != 0) and (call['DV'] >= 5) and (float(call['DV'])/float(call['DV']+call['DR'])>=altAF):
        ...
        if (len(rcRef) > 0) and (len(rcAlt) > 0) and (record.INFO['SVLEN'] >= minSize) and (record.INFO['SVLEN'] <= maxSize):
            ...
            if (args.svType == 'INV') or (record.INFO['SVLEN'] <= 10000) or ((args.svType == 'DEL') and (rdRatio <= 0.75)) or ((args.svType == 'DUP') and (rdRatio >= 1.25)) or (args.svType == 'TRA'):
